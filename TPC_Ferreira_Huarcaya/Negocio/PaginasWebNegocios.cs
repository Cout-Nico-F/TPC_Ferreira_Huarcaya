...
namespace Negocio
{
    public class PaginasWebNegocios
    {
        ...
        public int EnviarDatos(PaginaWeb pag)
        {
            ...
            ConexionMSSQL conexion = new ConexionMSSQL();
            int rowsAfectadas = conexion.SentenciaNonQuery("insert into PaginaWeb(Titulo,Descripcion,Url_Pagina,Url_Image,Habilitado,Precio) values ('" + pag.Titulo + "," + pag.Descripcion + "," + pag.Url_PaginaWeb + "," + pag.Url_Imagen +"')");
            conexion.Desconectar();
            return rowsAfectadas;
        }
